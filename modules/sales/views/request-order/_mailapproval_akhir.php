										...
										<b>
											<?php
												if($approval->status == 1)
													echo "Create Approval Request Order";
												else if($approval->status == 3)
													echo "Finish Approved Request Order";
												else
													echo "Finish Rejected Request Order";
											?>
										</b>
									</span>
								</div>
				...
				<table class="table-column" align="left" border="0" cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							<td valign="top">
								<span class="yth">
									<?= ($approval->status == 3) ? ucwords($approval->request->profile->name) .' Request Order Selesai di APPROVE' : ucwords($approval->request->profile->name) .' Request Order di REJECT' ?>
								</span>
								<div class="padcon">
									...
									<table class="table-column">
										<tbody>
											...
											<tr>
												...
												<td class="semicolon">:</td>
												<td><strong><?= isset($approval->request->profile) ? $approval->request->profile->name : '' ?></strong></td>
											</tr>
											<tr>
												<td>Tanggal</td>
												<td class="semicolon">:</td>
