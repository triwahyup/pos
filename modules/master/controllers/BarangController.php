        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                ...
                try{
                    ...
                    if(!$model->save()){
                        $success = false;
                        $message = (count($model->errors) > 0) ? 'ERROR CREATE BARANG: ' : '';
                    ...
                    if($success){
                        $transaction->commit();
                        $message = 'CREATE BARANG: '.$model->name;
                        $logs =	[
                            'type' => Logs::TYPE_USER,
                            'description' => $message,
                        ];
    ...
    public function actionUpdate($code)
    {
        ...
        if ($this->request->isPost && $model->load($this->request->post())) {
            ...
            try{
                if(!$model->save()){
                    $success = false;
                    $message = (count($model->errors) > 0) ? 'ERROR UPDATE BARANG: ' : '';
                    foreach($model->errors as $error => $value){
                        $message .= $value[0].', ';
                    }
                    $message = substr($message, 0, -2);
                ...
                if($success){
                    $transaction->commit();
                    $message = 'UPDATE BARANG: '.$model->name;
                    $logs =	[
                        'type' => Logs::TYPE_USER,
                        'description' => $message,
                    ];
    ...
    public function actionDelete($code)
    {
        ...
        if(isset($model)){
            ...
            try{
                ...
                if(!$model->save()){
                    $success = false;
                    $message = (count($model->errors) > 0) ? 'ERROR DELETE BARANG: ' : '';
                    foreach($model->errors as $error => $value){
                        $message .= $value[0].', ';
                    }
                    $message = substr($message, 0, -2);
                ...
                if($success){
                    $transaction->commit();
                    $message = 'DELETE BARANG:'. $model->name;
                    $logs =	[
                        'type' => Logs::TYPE_USER,
                        'description' => $message,
                    ];
