...
use app\modules\master\models\MasterEkspedisiSearch;
...
/**
 * EkspedisiController implements the CRUD actions for MasterPerson model.
 */
class EkspedisiController extends Controller
{
            ...
            [
                'access' => [
				    ...
				    'rules' => [
                        [
                            'actions' => ['create'],
                            'allow' => (((new User)->getIsDeveloper()) || \Yii::$app->user->can('data-ekspedisi')),
                            'roles' => ['@'],
                        ],
                        [
    ...
    {
        $searchModel = new MasterEkspedisiSearch();
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                ...
                try {
                    ...
                    $model->type_user = \Yii::$app->params['TYPE_EKSPEDISI'];
                    ...
                    if(!$model->save()){
                        $success = false;
                        $message = (count($model->errors) > 0) ? 'ERROR CREATE EKSPEDISI: ' : '';
                    ...
                    if($success){
                        $transaction->commit();
                        $message = '['.$model->code.'] SUCCESS CREATE EKSPEDISI.';
                        $logs =	[
                            'type' => Logs::TYPE_USER,
                            'description' => $message,
                        ];
    ...
    public function actionUpdate($code)
    {
        ...
        if ($this->request->isPost && $model->load($this->request->post())) {
            ...
            try {
                ...
                if(!$model->save()){
                    $success = false;
                    $message = (count($model->errors) > 0) ? 'ERROR UPDATE EKSPEDISI: ' : '';
                    foreach($model->errors as $error => $value){
                        $message .= $value[0].', ';
                    }
                    $message = substr($message, 0, -2);
                ...
                if($success){
                    $transaction->commit();
                    $message = '['.$model->code.'] SUCCESS UPDATE EKSPEDISI.';
                    $logs =	[
                        'type' => Logs::TYPE_USER,
                        'description' => $message,
                    ];
    ...
    public function actionDelete($code)
    {
        ...
        if(isset($model)){
            ...
            try{
                ...
                if(!$model->save()){
                    $success = false;
                    $message = (count($model->errors) > 0) ? 'ERROR DELETE EKSPEDISI: ' : '';
                    foreach($model->errors as $error => $value){
                        $message .= $value[0].', ';
                    }
                    $message = substr($message, 0, -2);
                ...
                if($success){
                    $transaction->commit();
                    $message = '['.$model->code.'] SUCCESS DELETE EKSPEDISI.';
                    \Yii::$app->session->setFlash('success', $message);
                }else{
                    $transaction->rollBack();
                    \Yii::$app->session->setFlash('error', $message);
