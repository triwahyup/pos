										...
										<b>
											<?php
												if($approval->status == 1)
													echo "Create Approval Purchase Internal";
												else if($approval->status == 3)
													echo "Finish Approved Purchase Internal";
												else
													echo "Finish Rejected Purchase Internal";
											?>
										</b>
									</span>
								</div>
				...
				<table class="table-column" align="left" border="0" cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							<td valign="top">
								<span class="yth">
									<?= ($approval->status == 3) ? ucwords($approval->poInternal->profile->name) .' PO Internal Selesai di APPROVE' : ucwords($approval->poInternal->profile->name) .' PO Internal di REJECT' ?>
								</span>
								<div class="padcon">
									...
									<table class="table-column">
										<tbody>
											...
											<tr>
												...
												<td class="semicolon">:</td>
												<td><strong><?= isset($approval->poInternal->profile) ? $approval->poInternal->profile->name : '' ?></strong></td>
											</tr>
											<tr>
												<td>Tanggal</td>
												<td class="semicolon">:</td>
