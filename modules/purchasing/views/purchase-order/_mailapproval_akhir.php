										...
										<b>
											<?php
												if($approval->status == 1)
													echo "Create Approval Purchase Order";
												else if($approval->status == 3)
													echo "Finish Approved Purchase Order";
												else
													echo "Finish Rejected Purchase Order";
											?>
										</b>
									</span>
								</div>
				...
				<table class="table-column" align="left" border="0" cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							<td valign="top">
								<span class="yth">
									<?= ($approval->status == 3) ? ucwords($approval->po->profile->name) .' PO Selesai di APPROVE' : ucwords($approval->po->profile->name) .' PO di REJECT' ?>
								</span>
								<div class="padcon">
									...
									<table class="table-column">
										<tbody>
											...
											<tr>
												...
												<td class="semicolon">:</td>
												<td><strong><?= isset($approval->po->profile) ? $approval->po->profile->name : '' ?></strong></td>
											</tr>
											<tr>
												<td>Tanggal</td>
												<td class="semicolon">:</td>
