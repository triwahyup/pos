										...
										<b>
											<?php
												if($approval->status == 1)
													echo "Create Approval Opname";
												else if($approval->status == 3)
													echo "Finish Approved Opname";
												else
													echo "Finish Rejected Opname";
											?>
										</b>
									</span>
								</div>
				...
				<table class="table-column" align="left" border="0" cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							<td valign="top">
								<span class="yth">
									<?= ($approval->status == 3) ? ucwords($approval->opname->profile->name) .' Opname Selesai di APPROVE' : ucwords($approval->opname->profile->name) .' Opname di REJECT' ?>
								</span>
								<div class="padcon">
									...
									<table class="table-column">
										<tbody>
											...
											<tr>
												...
												<td class="semicolon">:</td>
												<td><strong><?= isset($approval->opname->profile) ? $approval->opname->profile->name : '' ?></strong></td>
											</tr>
											<tr>
												<td>Tanggal</td>
												<td class="semicolon">:</td>
